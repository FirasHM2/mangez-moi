...
$(document).ready(function() {
    ...
    $table = $('#data-table-breads').DataTable({
        searching: true,
        ordering: false,
        order:[[0, 'desc']],
        paging:false,
...
$("#add").click(function() {
    $.post(siteUrl + '/admin-panel/breads/add', {
        data: {
        ...
        if (id.status) {
            Materialize.toast(id.msg, 2000, "red");
            return
        }
        ...
        $($(nRow).find('button[name="delete"]')[0]).click(function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    $.post(siteUrl + '/admin-panel/breads/delete', {
                        _id: _id,
                    }, function (data) {
                        if (data == "Success") {
        ...
        $($(nRow).find('button[name="save"]')[0]).click(function () {
            ...
            $.post(siteUrl + '/admin-panel/breads/update', {
                _id: _id,
                data: {
                    name: name,
                    price: price
